...
# Context:
你正在分析一条社交媒体上的纯文本post。你的任务是为其生成一个“文本深度洞察摘要”。这个摘要将作为后续宏观分析的输入。

# Input:
- Post 文本: ```{post_text}```

# Your Task:
请仔细阅读Post文本，深入理解后，输出一段“文本深度洞察摘要”。摘要需自然地融合以下三个层面(如果可以识别到)：
1.  **核心信息**: Post讨论的主要观点和关键信息是什么？
2.  **情绪意图**: 作者展现了怎样的情绪和语气？他/她发文的潜在动机是什么？
3.  **思想价值**: 这条Post试图引发读者怎样的思考或共鸣？"""
